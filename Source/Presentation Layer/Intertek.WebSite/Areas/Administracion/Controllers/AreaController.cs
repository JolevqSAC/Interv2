    ...
    public class AreaController : BaseController
    {
        ...
        public virtual JsonResult Listar(GridTable grid)
        {
            try
            {
                ...
                var generic = Listar(AreaBL.Instancia, grid.sidx, grid.sord, grid.page, grid.rows, grid._search, grid.searchField, grid.searchOper, grid.searchString, where);
                ...
                generic.Value.rows = generic.List
                    .Select(item => new Row
                    {
                        ...
                        cell = new[]
                            {
                                item.IDArea.ToString(),
                                item.ARE_Codigo,
                                item.ARE_Nombre,
                                item.ARE_Descripcion,
                                item.NombreEstado
                            }
                    }).ToArray();
        ...
        public virtual ActionResult Modificar(string id)
        {
            try
            {
                var entidad = AreaBL.Instancia.Single(Convert.ToInt32(id));
        ...
        [HttpPost]
        public virtual JsonResult Modificar(Area entidad)
        {
            var jsonResponse = new JsonResponse { Success = false };
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    entidad.UsuarioModificacion = UsuarioActual.IdUsuario.ToString();
                    AreaBL.Instancia.Update(entidad);
                    jsonResponse.Success = true;
                    jsonResponse.Message = "Se Proceso con éxito";
                }
                catch (Exception ex)
        ...
        public virtual JsonResult ListarAreas()
        {
            try
            {
                var areas = AreaBL.Instancia.GetAll();
                var cadenaFormateada = string.Empty;
                foreach (var item in areas)
                {
                    cadenaFormateada += string.Format("<option value='{0}'>{1}</option>", item.IDArea, item.ARE_Nombre);
                }
        ...
        private void PrepararDatos(ref Area entidad, string accion)
        {
