...
namespace Intertek.WebSite.Areas.Administracion.Controllers
{
    ...
    [Authorize]
    public class MetodoController : BaseController
    {
        //
        // GET: /Administracion/Metodo/
        ...
        public virtual JsonResult Listar(GridTable grid)
        {
            try
            {
                ...
                var generic = Listar(MetodoBL.Instancia, grid.sidx, grid.sord, grid.page, grid.rows, grid._search, grid.searchField, grid.searchOper, grid.searchString, where);
                ...
                generic.Value.rows = generic.List
                    .Select(item => new Row
                    {
                        ...
                        cell = new[]
                            {
                                item.IDMetodo.ToString(),
                                item.MET_Codigo,
                                item.MET_Nombre,
                                item.MET_NombreIngles,
                                item.MET_Descripcion,
                                item.NombreEstado
                            }
                    }).ToArray();
        ...
        public virtual ActionResult Modificar(string id)
        {
            try
            {
                var entidad = MetodoBL.Instancia.Single(Convert.ToInt32(id));
        ...
        [HttpPost]
        public virtual JsonResult Modificar(Metodo entidad)
        {
            var jsonResponse = new JsonResponse { Success = false };
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (entidad.MET_Descripcion == null)
                        entidad.MET_Descripcion = "";
                    // fin dato
                    MetodoBL.Instancia.Update(entidad);
                    jsonResponse.Success = true;
                    jsonResponse.Message = "Se Proceso con éxito";
                }
                catch (Exception ex)
        ...
        [HttpPost]
        public virtual JsonResult ListarMetodos()
        {
            try
            {
                var areas = MetodoBL.Instancia.GetAll();
                var cadenaFormateada = string.Empty;
                foreach (var item in areas)
                {
                    cadenaFormateada += string.Format("<option value='{0}'>{1}</option>", item.IDMetodo, item.MET_Nombre);
                }
