...
namespace Intertek.WebSite.Areas.Administracion.Controllers
{
    ...
    {
        //
        // GET: /Administracion/CentroCosto/
        ...
        public virtual ActionResult Modificar(string id)
        {
            try
            {
                var entidad = CentroCostoBL.Instancia.Single(Convert.ToInt32(id));
        ...
        [HttpPost]
        public virtual JsonResult Modificar(CentroCosto entidad)
        {
            var jsonResponse = new JsonResponse { Success = false };
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    entidad.UsuarioModificacion = UsuarioActual.IdUsuario.ToString();
                    CentroCostoBL.Instancia.Update(entidad);
                    jsonResponse.Success = true;
                    jsonResponse.Message = "Se Proceso con éxito";
                }
                catch (Exception ex)
        ...
        [HttpPost]
        public virtual JsonResult ListarCentroCostos()
        {
            try
            {
                var areas = CentroCostoBL.Instancia.GetAll();
                var cadenaFormateada = string.Empty;
                foreach (var item in areas)
                {
                    cadenaFormateada += string.Format("<option value='{0}'>{1}</option>", item.IDCentroCosto, item.CCO_Nombre);
                }
