    ...
    public class ProductoController : BaseController
    {
        ...
        public virtual JsonResult Listar(GridTable grid)
        {
            try
            {
                ...
                var generic = Listar(ProductoBL.Instancia, grid.sidx, grid.sord, grid.page, grid.rows, grid._search, grid.searchField, grid.searchOper, grid.searchString, where);
        ...
        public virtual ActionResult Modificar(string id)
        {
            try
            {
                var entidad = ProductoBL.Instancia.Single(Convert.ToInt32(id));
        ...
        [HttpPost]
        public virtual JsonResult Modificar(Producto entidad)
        {
            var jsonResponse = new JsonResponse { Success = false };
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    entidad.UsuarioModificacion = UsuarioActual.IdUsuario.ToString();
                    ProductoBL.Instancia.Update(entidad);
                    jsonResponse.Success = true;
                    jsonResponse.Message = "Se Proceso con éxito";
                }
                catch (Exception ex)
        ...
        [HttpPost]
        public virtual JsonResult ListarProductos()
        {
            try
            {
                var areas = ProductoBL.Instancia.GetAll();
                var cadenaFormateada = string.Empty;
                foreach (var item in areas)
                {
                    cadenaFormateada += string.Format("<option value='{0}'>{1}</option>", item.IDProducto, item.PRO_Nombre);
                }
        ...
        private void PrepararDatos(ref Producto entidad, string accion)
        {
